...
Capacitor 是 Ionic 的官方应用运行时，可以轻松将 Web 应用部署到 iOS、Android 等原生平台。如果你以前使用过 Cordova，请考虑[在此处](https://capacitorjs.com/docs/cordova#differences-between-capacitor-and-cordova)阅读更多关于差异的信息。
...
为了使某些原生插件能够工作，必须配置用户权限。在我们的相册应用中，这包括 Camera 插件：iOS 在首次调用 `Camera.getPhoto()` 后会自动显示一个模态对话框，提示用户允许应用使用相机。驱动此功能的权限标记为"Privacy - Camera Usage"。要设置它，需要修改 `Info.plist` 文件（[更多详情请点击这里](https://capacitorjs.com/docs/ios/configuration)）。要访问它，请单击"Info"，然后展开"Custom iOS Target Properties"。
