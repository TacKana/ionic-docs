...
<head>
  ...
  <meta
    name="description"
    content="使用 Capacitor 为 Ionic Framework 应用设置部署到 iOS 和 Android 非常简单。阅读我们的 Ionic 文档了解更多关于部署的信息。"
  />
</head>

由于我们在创建项目时就已经添加了 Capacitor，因此相册应用到达我们的设备仅需几个步骤！

## Capacitor 设置

Capacitor 是 Ionic 的官方应用运行时，可以轻松将 Web 应用部署到 iOS、Android 等原生平台。如果你以前使用过 Cordova，请考虑阅读关于 [Capacitor 与 Cordova 之间的差异](https://capacitorjs.com/docs/cordova#differences-between-capacitor-and-cordova)的更多内容。

如果你仍在终端中运行 `ionic serve`，请取消它。完成 Ionic 项目的全新构建，并修复它报告的任何错误：
...
在项目根目录下会创建 android 和 ios 文件夹。这些是完全独立的本机项目，应被视为 Ionic 应用的一部分（即，将它们检入源代码控制，使用本机工具编辑它们等）。

每次执行构建（例如 `ionic build`）更新 Web 目录（默认：`build`）时，都需要将这些更改复制到本机项目中：

```shell
ionic cap copy
```

注意：在对代码的本机部分进行更新（例如添加新插件）之后，请使用 `sync` 命令：
...
为了使某些原生插件能够工作，必须配置用户权限。在我们的相册应用中，这包括 Camera 插件：iOS 在首次调用 `Camera.getPhoto()` 后会自动显示一个模态对话框，提示用户允许应用使用相机。驱动此功能的权限标记为"Privacy - Camera Usage"。要设置它，需要修改 `Info.plist` 文件（[更多关于 iOS 配置的详细信息](https://capacitorjs.com/docs/ios/configuration)）。要访问它，请单击"Info"，然后展开"Custom iOS Target Properties"。
...
`Info.plist` 中的每个设置都有一个低级参数名和一个高级名称。默认情况下，属性列表编辑器显示高级名称，但切换到显示原始的低级名称通常很有用。为此，请在属性列表编辑器中的任意位置右键单击，然后切换"Raw Keys/Values"。

添加 `NSCameraUsageDescription` 键，并将值设置为描述应用需要使用相机的原因，例如"To Take Photos"。当权限提示打开时，值字段会显示给应用用户。

按照相同的过程添加 Camera 插件所需的其他两个键：`NSPhotoLibraryAddUsageDescription` 和 `NSPhotoLibraryUsageDescription`。

接下来，单击左侧 Project Navigator 中的 `App`，然后在 `Signing & Capabilities` 部分中选择你的 Development Team。
...
权限设置完成并选择了 Development Team 后，我们就可以在真实设备上尝试该应用了！将 iOS 设备连接到 Mac 电脑，选择它（对我来说是 `App -> Matthew's iPhone`），然后单击"Build"按钮来构建、安装和启动设备上的应用：
...
保存文件。权限设置完成后，我们就可以在真实设备上尝试该应用了！将 Android 设备连接到计算机。在 Android Studio 中，单击"Run"按钮，选择连接的 Android 设备，然后单击 OK 来构建、安装和启动设备上的应用。

![Android Studio 界面，箭头指向 Run 按钮和连接的设备。](/img/guides/first-app-cap-ng/android-device.png 'Android Studio Run 配置')

再次，点击相册标签页上的相机按钮后，将显示权限提示。点击 OK，然后用相机拍照。之后，照片会出现在应用中。
