...
<head>
  ...
  <meta
    name="description"
    content="Ionic Vue 快速入门涵盖 Vue 和 Ionic 的基础知识，包括任何 Ionic 特定的功能。了解如何使用 Ionic CLI 构建 Vue 应用。"
  />
</head>
...
如果您想了解 Ionic Vue 是什么以及它如何融入 Vue 生态系统的高级概述，请参阅 [Ionic Vue 概述](overview)。
...
您可以使用更多 Ionic UI 组件来增强首页。例如，在 `ion-content` 的末尾添加一个 [Button](/api/button.md)：
...
:::warning
创建自己的页面时，始终使用 `ion-page` 作为根组件。这对于页面之间的正确过渡、Ionic 组件依赖的基础 CSS 样式以及应用中一致的布局行为至关重要。
:::

## 导航到新页面

要导航到新页面，先为其创建路由。在 `router/index.ts` 中 `HomePage` 导入之后添加导入：
...
:::info
导航也可以通过 Vue Router 以编程方式执行，路由可以延迟加载以获得更好的性能。更多信息请参阅 [Vue 导航文档](/vue/navigation.md)。
:::
...
更新 `NewPage.vue`，在 `ion-content` 上添加一个 ref，在现有图标之后添加一个按钮和一些项目：
...
这种模式是必要的，因为 Ionic 组件是作为 Web Component 构建的。`$el` 属性让您可以访问定义了方法的实际 Web Component 实例。
...
6. 移除 Vue 组件中 `<script>` 标签上的 `lang="ts"`。在一个空白的 Ionic Vue 应用中，这应该只涉及 `src/App.vue` 和 `src/views/HomePage.vue`。
...
本指南涵盖了创建 Ionic Vue 应用、添加导航以及介绍用于原生构建的 Capacitor 的基础知识。要深入学习，请查阅：
