...
<head>
  ...
  <meta
    name="description"
    content="使用 Vue 构建的 Ionic 应用中有多种存储数据的方式可供选择。选择最适合您存储需求的选项。"
  />
</head>

在 Ionic 应用中有多种存储数据的方式可供选择。最好选择最适合您应用需求的选项。单个应用可能有跨越多个选项的需求。
...
某些应用，尤其是遵循离线优先方法论的应用，可能需要在本地存储大量复杂的关系型数据。对于此类场景，可以使用 SQLite 插件。最常见的 SQLite 插件产品包括：

- [Cordova SQLite Storage](https://github.com/storesafe/cordova-sqlite-storage)（此插件也有一个[便捷封装器](https://danielsogl.gitbook.io/awesome-cordova-plugins/sqlite)以辅助实现）
- [Capacitor Community SQLite Plugin](https://github.com/capacitor-community/sqlite)

## 非关系型大量数据存储（移动端和 Web）

对于需要存储大量数据以及在 Web 和移动端上运行的应用程序，一个潜在的解决方案是创建一个键/值对数据存储服务，在 Web 上使用 [indexedDB](https://developer.mozilla.org/en-US/docs/Web/API/IndexedDB_API)，在移动端使用前面提到的 SQLite 插件之一。
...
其他提供本地以及基于云存储的存储选项也适用于 Capacitor 应用，并且可能与您的应用很好地集成。
