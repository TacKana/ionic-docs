...
只要导入了 [typography.css](/layout/global-stylesheets#typographycss) 文件，动态字体缩放默认就是启用的。导入此文件将定义 `--ion-dynamic-font` 变量，从而激活动态字体缩放。虽然不推荐，但开发者可以通过在应用代码中将此变量设置为 `initial` 来选择退出动态字体缩放。
...
2.  确认已导入 [typography.css](/layout/global-stylesheets#typographycss) 文件。该文件是动态字体缩放正常工作所必需的。
