...
这将在 [`:root`](https://developer.mozilla.org/en-US/docs/Web/CSS/:root) 选择器上设置[应用颜色](/theming/themes#application-colors)和[阶梯颜色](/theming/themes#stepped-colors)。
...
这将在 [CSS 媒体查询 `prefers-color-scheme`](https://developer.mozilla.org/en-US/docs/Web/CSS/@media/prefers-color-scheme) 为 `dark` 时设置[应用颜色](/theming/themes#application-colors)和[阶梯颜色](/theming/themes#stepped-colors)。`prefers-color-scheme` 媒体查询得到[所有现代浏览器](https://caniuse.com/#feat=prefers-color-scheme)的支持。如果需要支持旧版浏览器，我们建议使用 [CSS 类](#css-class)方法。
...
这将在 `.ion-palette-dark` 选择器上设置[应用颜色](/theming/themes#application-colors)和[阶梯颜色](/theming/themes#stepped-colors)，该类必须由开发者应用到应用中。
