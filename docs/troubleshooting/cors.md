...
<head>
  ...
  <meta
    name="description"
    content="当 Web 应用发起请求而服务器未返回必需的头部时，就会发生 CORS 错误。请在 Ionic 文档中阅读关于跨域资源共享的内容。"
  />
</head>
...
**源**是提供 Ionic 应用程序或外部资源的**协议**、**域名**和**端口**的组合。例如，在 Capacitor 中运行的应用程序将 `capacitor://localhost`（iOS）或 `http://localhost`（Android）作为其源。

当提供应用程序的源（例如使用 `ionic serve` 时的 `http://localhost:8100`）与所请求资源的源（例如 `https://api.example.com`）不匹配时，浏览器的<a href="https://developer.mozilla.org/en-US/docs/Web/Security/Same-origin_policy" target="_blank" rel="noopener">同源策略</a>生效，并且请求需要 CORS 才能发出。
...
默认情况下，当 Web 应用程序尝试发出跨域请求时，浏览器会在实际请求之前发送一个**预检请求**。需要此预检请求来了解外部资源是否支持 CORS 以及是否可以安全地发送实际请求，因为它可能会影响用户数据。
...
- **或者具有以下 `Content-Type` 头部之外的 `Content-Type` 头部：**
  ...
  - multipart/form-data
  - text/plain
- 或者如果使用了 `ReadableStream` 或 `XMLHttpRequestUpload` 中的事件监听器。

如果满足上述任何条件，则会向资源 URL 发送一个使用 `OPTIONS` 方法的预检请求。

假设我们向位于 `https://api.example.com` 的虚构 JSON API 发出一个 `POST` 请求，其 `Content-Type` 为 `application/json`。预检请求将如下所示（为清晰起见，省略了一些默认头部）：
...
如果服务器启用了 CORS，它将解析 `Access-Control-Request-*` 头部，并理解尝试从 `http://localhost:8100` 发出一个带有自定义 `Content-Type` 的 `POST` 请求。
...
| Header                           | Value             | 描述                                                                                                                                                                                          |
| -------------------------------- | ----------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Access-Control-Allow-Origin**  | `origin` 或 `*`   | 指定允许的源，例如 `http://localhost:8100` 或 `*` 允许所有源。                                                                                                                                 |
| **Access-Control-Allow-Methods** | `methods`         | 访问资源时允许的方法：`GET`、`HEAD`、`POST`、`PUT`、`DELETE`、`CONNECT`、`OPTIONS`、`TRACE`、`PATCH`。                                                                                         |
| **Access-Control-Allow-Headers** | `headers`         | 用于响应预检请求，指示在发出实际请求时可以使用哪些头部（除了[简单头部](#simple-requests)之外，简单头部始终允许）。                                                                             |
| Access-Control-Allow-Credentials | `true` 或 `false` | 是否可以携带凭据发出请求。                                                                                                                                                                    |
| Access-Control-Expose-Headers    | `headers`         | 指定浏览器允许访问的头部。                                                                                                                                                                    |
...
| Header                            | Value    | 描述                                         |
| --------------------------------- | -------- | -------------------------------------------- |
| **Access-Control-Request-Method** | `method` | 用于让服务器知道在发出实际请求时将使用什么方法。 |
| Access-Control-Request-Headers    | `headers` | 用于让服务器知道在发出实际请求时将使用哪些非简单头部。 |

## CORS 错误的解决方案

### A. 在您控制的服务器上启用 CORS

正确且最简单的解决方案是通过从 Web 服务器或后端返回[正确的响应头部](#server-headers-response-)，并响应预检请求来启用 CORS，因为这样可以继续使用 `XMLHttpRequest`、`fetch` 或 Angular 中的 `HttpClient` 等抽象。
...
| `npm run start` 或 `ng serve` | Ionic Angular 应用程序的 `http://localhost:4200`。   |
...
有关如何在不同的 Web 和应用程序服务器中启用 CORS 的更多信息，请查看 <a href="https://enable-cors.org" target="_blank" rel="noopener">enable-cors.org</a>

在 Express/Connect 应用程序中，可以使用 <a href="https://github.com/expressjs/cors" target="_blank" rel="noopener">cors</a> 中间件轻松启用 CORS：
...
对于 Capacitor 应用程序，请使用 [Capacitor HTTP API](https://capacitorjs.com/docs/apis/http)。此 API 会修补 `fetch` 和 `XMLHttpRequest` 以使用原生库。请注意，如果您还将应用程序部署到基于 Web 的环境（例如 PWA 或本地开发服务器，例如通过 `ionic serve`），您仍然需要为这些场景实现 CORS。

##### 传统 Cordova 应用程序

对于传统的 Cordova 应用程序，请使用 [带有 Awesome Cordova Plugins 包装器的 HTTP 插件](https://danielsogl.gitbook.io/awesome-cordova-plugins/http)。请注意，此插件在浏览器中不起作用，因此必须始终在设备或模拟器上进行应用程序的开发和测试。
...
查看 <a href="https://github.com/Rob--W/cors-anywhere/" target="_blank" rel="noopener">cors-anywhere</a>，这是一个可以在您自己的服务器上部署的 Node.js CORS 代理。不建议在生产环境中使用免费的托管 CORS 代理。
