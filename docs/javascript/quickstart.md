...
<head>
  <title>Ionic JavaScript 快速入门：JavaScript 基础</title>
  <meta
    name="description"
    content="Ionic JavaScript 快速入门涵盖 JavaScript 和 Ionic 的基础知识，包括任何 Ionic 特定的功能。了解如何构建 JavaScript 应用。"
  />
</head>
...
欢迎！本指南将带领您了解使用 Vite 进行 Ionic JavaScript 开发的基础知识。您将学习如何设置开发环境、创建简单项目、探索项目结构以及理解 Ionic 组件的工作原理。这是在构建第一个真实应用之前熟悉 Ionic JavaScript 的完美方式。

如果您想了解 Ionic JavaScript 是什么以及它如何融入 Web 开发生态系统的高级概述，请参阅 [Ionic JavaScript 概述](overview)。
...
首先，使用 vanilla JavaScript 创建一个新的 Vite 项目：
...
在项目根目录创建一个 `vite.config.js` 文件，内容如下：
...
这会复制 Capacitor 处理延迟加载的 Ionic 组件所需的必要 Ionic 文件。
...
这设置了应用的根结构，使用 Ionic 的 `ion-app`、`ion-router` 和 `ion-router-outlet` 组件。关键的更改是将默认的 `<div id="app">` 替换为 Ionic 的路由和布局组件。路由出口是页面显示的位置。

## 查看路由

路由在 `index.html` 中使用 `ion-router` 内部的 `ion-route` 组件定义：
...
当您访问根 URL (`/`) 时，将加载 `home-page` 组件。当您访问 `/new` URL 时，将加载 `new-page` 组件。我们接下来将定义这些组件。
...
您可以使用更多 Ionic UI 组件来增强首页。例如，添加一个 [Button](/api/button.md) 来导航到另一个页面。更新 `HomePage.js` 中的 `HomePage` 组件：
...
这会创建一个在 [Toolbar](/api/toolbar.md) 中包含[返回按钮](/api/back-button.md)的页面。返回按钮会自动处理导航到上一页，如果没有历史记录则导航到 `/`。

接下来，更新 `index.html` 中导入首页的 `<script>` 标签，同时导入新页面：
...
要导航到新页面，将 `HomePage.js` 中的按钮放在 `ion-router-link` 内部：
...
:::info
导航也可以使用 `document.querySelector('ion-router').push('/new')` 以编程方式执行。有关更多信息，请参阅 [Ionic Router 文档](/api/router.md)。
:::

## 为新页面添加图标

Ionic JavaScript 支持 [Ionicons](https://ionic.io/ionicons/)。要使用图标，您需要导入它们，使用 `addIcons` 注册它们，然后使用 `ion-icon` 组件。
...
您可以在每个组件的 API 文档的 [Methods](/api/content.md#methods) 部分找到可用的方法。
...
Ionic Core 也适用于其他支持自定义元素的框架和库，例如 [Alpine.js](https://alpinejs.dev/)、[Lit](https://lit.dev/) 和 [Svelte](https://svelte.dev/)。但是，在将这些库与 Ionic Core 一起使用时，您将无法使用与 Ionic 官方 Angular、React 和 Vue 框架集成紧密耦合的内置表单和路由功能，并且需要使用它们各自的路由和表单解决方案。

## 探索更多

本指南涵盖了使用 Vite 创建 Ionic JavaScript 应用、添加导航以及介绍用于原生构建的 Capacitor 的基础知识。要深入学习，请查阅：

<DocsCards>

<DocsCard header="Vite 文档" href="https://vite.dev/guide/" icon="/icons/logo-vite-icon.png">
  <p>了解 Ionic JavaScript 应用的高级 Vite 配置和优化技术。</p>
</DocsCard>
