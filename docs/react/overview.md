...
<head>
  <title>Ionic React 概述 | React 版本支持与工具</title>
  <meta
    name="description"
    content="@ionic/react 将 Ionic 体验与专为 React 开发者打造的工具和 API 相结合。在我们的 React 概述中了解更多关于版本支持和资源的信息。"
  />
</head>
...
`@ionic/react` 将 Ionic Framework 的全部能力带给 React 开发者。它与 React 生态系统无缝集成，让你可以使用熟悉的 React 工具、组件和最佳实践来构建高质量的跨平台应用。同时你还可以使用 Ionic 丰富的 UI 库和原生能力。
...
Ionic React 与 React CLI 及流行的 React 工具无缝协作。你可以使用你喜欢的库进行状态管理、测试等。Ionic React 设计上自然融入 React 生态系统，因此你可以使用 Create React App、Vite 或 Next.js 等工具来搭建和构建你的应用。

## 原生工具

[Capacitor](https://capacitorjs.com) 是 Ionic Angular 的官方跨平台运行时，让你可以仅用单一代码库在 iOS、Android 和 Web 上原生运行应用。
...
<DocsCard header="入门指南" href="quickstart" icon="/icons/guide-quickstart-icon.png">
  <p>快速搭建你的第一个 Ionic React 应用，学习框架和 CLI 的基础知识。</p>
</DocsCard>
