...
`isPlatform` 方法可用于测试你的应用是否在某个特定平台上运行：
...
根据用户所在的平台，isPlatform(platformName) 将返回 true 或 false。请注意，同一个应用可能对多个平台名称返回 true。例如，在 iPad 上运行的应用将对以下平台名称返回 true：mobile、ios、ipad 和 tablet。此外，如果应用是从 Cordova 运行的，则 cordova 也会为 true。
...
根据你所使用的设备，`getPlatforms` 可以返回多个值。每个可能的值都是一个平台层级结构。例如，在 iPhone 上，它将返回 mobile、ios 和 iphone。

## 平台列表

以下表格列出了所有可能的平台值及其对应描述。
...
检测特定平台的函数可以通过在全局 [Ionic 配置](../developing/config)中提供替代函数来覆盖。每个函数以 `window` 作为参数并返回一个布尔值。

```tsx
setupIonicReact({
  platform: {
    /** 默认的 `desktop` 函数对带有触摸屏的设备返回 false。
     * 这并不总是我们想要的，所以此函数改用 User Agent 进行检测。
     **/
    desktop: (win) => {
