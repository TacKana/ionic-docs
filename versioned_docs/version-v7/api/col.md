...
<head>
  ...
  <meta
    name="description"
    content="ion-col 是一个位于行内部的列组件。网格内的内容应放置在列中。了解更多关于列内边距、尺寸及其他属性的信息。"
  />
</head>
...
列是[网格](./grid)系统的单元格组件，位于[行](./row)内部。它们会扩展以填充整行。网格中的所有内容都应放置在列内。

更多信息，请参阅[网格](./grid)文档。

## 列对齐方式

默认情况下，列会拉伸以填满行的整个高度。列是[弹性项目（Flex Item）](https://developer.mozilla.org/en-US/docs/Glossary/Flex_Item)，因此可以使用多个 [CSS 类](/docs/layout/css-utilities#flex-item-properties)应用于列以自定义此行为。
