...
默认情况下，列会拉伸以填满整行的高度，并在必要时换行。行是[弹性容器](https://developer.mozilla.org/zh-CN/docs/Glossary/Flex_Container)，因此有多种[CSS 类](/v7/layout/css-utilities#flex-属性)可应用于行来自定义此行为。
