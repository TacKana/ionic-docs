...
<head>
  ...
  <meta
    name="description"
    content="ion-route 组件接收一个组件，并在浏览器 URL 匹配 url 属性时渲染该组件。了解 Ionic 应用中 API 路由组件的更多信息。"
  />
</head>
...
:::note
注意：此组件应仅用于原生 JavaScript 和 Stencil 项目。对于 Angular 项目，请使用 [`ion-router-outlet`](router-outlet.md) 和 Angular 路由器。
:::

## 导航钩子

导航钩子可用于执行任务或充当导航守卫。通过为每个 `ion-route` 的 `beforeEnter` 和 `beforeLeave` 属性提供函数来使用钩子。返回 `true` 允许导航继续，而返回 `false` 则取消导航。返回 `NavigationHookOptions` 类型的对象允许你将导航重定向到另一页面。
