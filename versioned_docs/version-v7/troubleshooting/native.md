...
<head>
  ...
  <meta
    name="description"
    content="探讨原生应用错误，包括代码签名、Xcode 构建错误 65 以及冲突的 Google Play Services 版本。学习如何解决 Ionic 应用中的原生错误。"
  />
</head>
...
<!-- prettier-ignore -->
3. <strong>在 <b>项目导航器</b>（Project navigator）中，选择项目根目录以打开项目编辑器。在 **Identity** 部分，确认设置的 Package ID 与 Bundle Identifier 一致。</strong>
...
<!-- prettier-ignore -->
4. <strong>在同一项目编辑器的 <b>Signing</b> 部分，确保 <b>Automatically manage signing</b> 已启用。</strong> 然后选择一个开发团队（Development Team）。选定开发团队后，Xcode 将尝试自动配置描述文件和签名。

   ![Xcode 显示 Signing 部分，'Automatically manage signing' 已启用，并已选择一个开发团队。](/img/running/ios-xcode-signing-setup.png "Xcode Signing 部分")
...
此错误是 Xcode 返回的错误代码，可能由配置问题或过时的 cordova 依赖引起。要解决此错误，首先确保已使用上述说明生成了配置文件，然后尝试[从 Xcode 运行应用](../developing/ios.md#running-with-xcode)。
...
此错误是由于两个不同的插件尝试使用不同版本的 `Google Play Services` 引起的。要解决此问题，请确保您运行的是 `cordova` 版本 `7.1.0` 或更高，以及 `cordova-android` `6.3.0` 或更高。要安装最新的 `cordova`，请运行：
...
依赖 `Google Play Services` 的插件现在可以更新为使用相同版本。例如，如果 `pluginA` 使用版本 11.0，而 `pluginB` 使用版本 15.0，可以通过在 `config.xml` 文件中添加以下片段来将它们更新为相同版本：
