...
`isPlatform` 方法可用于检测你的应用是否运行在特定平台上：
...
根据用户当前所在的平台，`isPlatform(platformName)` 会返回 true 或 false。需要注意的是，同一个应用可能对多个平台名称都返回 true。例如，在 iPad 上运行的应用会对以下平台名称返回 true：mobile、ios、ipad 和 tablet。此外，如果应用是通过 Cordova 运行的，那么 cordova 也会返回 true。
