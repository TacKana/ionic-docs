...
Ionic Framework 提供了一组 CSS 工具类，可用于任何元素上，以修改文本、元素放置或调整内边距和外边距。

:::important
如果您的应用不是使用可用的 Ionic Framework 启动器创建的，则需要包含[全局样式表的可选部分](global-stylesheets.md#optional)中列出的样式表，这些样式才能生效。
:::
...
| 类                     | 样式规则                    | 描述                                                           |
| ---------------------- | --------------------------- | -------------------------------------------------------------- |
| `.ion-text-left`       | `text-align: left`          | 内联内容对齐到行框的左边缘。                                   |
| `.ion-text-right`      | `text-align: right`         | 内联内容对齐到行框的右边缘。                                   |
| `.ion-text-start`      | `text-align: start`         | 如果方向从左到右则等同于 `text-left`，如果方向从右到左则等同于 `text-right`。 |
| `.ion-text-end`        | `text-align: end`           | 如果方向从左到右则等同于 `text-right`，如果方向从右到左则等同于 `text-left`。   |
| `.ion-text-center`     | `text-align: center`        | 内联内容在行框内居中。                                         |
| `.ion-text-justify`    | `text-align: justify`       | 内联内容两端对齐。文本应均匀分布，使其左右边缘与行框的左右边缘对齐，最后一行除外。 |
| `.ion-text-wrap`       | `white-space: normal`       | 空白序列被折叠。源中的换行符与其他空白一样处理。必要时断开行以填充行框。 |
| `.ion-text-nowrap`     | `white-space: nowrap`       | 与 `normal` 一样折叠空白，但禁止文本内的换行（文本换行）。         |
...
上面列出的所有文本类都有额外的类，可以根据屏幕大小修改文本。将每个类中的 `text-` 替换为 `text-{breakpoint}-`，以仅在特定屏幕尺寸上使用该类，其中 `{breakpoint}` 是 [Ionic 断点](#ionic-breakpoints)中列出的断点名称之一。

下表显示了默认行为，其中 `{modifier}` 是以下任意一种：`left`、`right`、`start`、`end`、`center`、`justify`、`wrap`、`nowrap`、`uppercase`、`lowercase` 或 `capitalize`，如上所述。
...
[float](https://developer.mozilla.org/en-US/docs/Web/CSS/float) CSS 属性指定元素应沿其容器的左侧或右侧放置，文本和内联元素将环绕它。这样，元素脱离了网页的正常流，但仍保持为流的一部分，与绝对定位相反。
...
| `.ion-float-start` | `float: left` / `float: right`  | 如果方向从左到右则等同于 `float-left`，如果方向从右到左则等同于 `float-right`。 |
| `.ion-float-end`   | `float: left` / `float: right`  | 如果方向从左到右则等同于 `float-right`，如果方向从右到左则等同于 `float-left`。 |

### 响应式浮动类

上面列出的所有浮动类都有额外的类，可以根据屏幕大小修改浮动。将每个类中的 `float-` 替换为 `float-{breakpoint}-`，以仅在特定屏幕尺寸上使用该类，其中 `{breakpoint}` 是 [Ionic 断点](#ionic-breakpoints)中列出的断点名称之一。

下表显示了默认行为，其中 `{modifier}` 是以下任意一种：`left`、`right`、`start` 或 `end`，如上所述。
...
[display](https://developer.mozilla.org/en-US/docs/Web/CSS/display) CSS 属性设置元素是作为块还是内联框处理，以及用于其子元素的布局，如流布局、网格或 flex。它也可以用于完全从布局中隐藏元素。
...
还有额外的类可以根据屏幕大小修改可见性。不要对所有屏幕尺寸使用 `.ion-hide`，而应使用 `.ion-hide-{breakpoint}-{dir}` 来仅在特定屏幕尺寸上使用该类，其中 `{breakpoint}` 是 [Ionic 断点](#ionic-breakpoints)中列出的断点名称之一，`{dir}` 表示元素应在指定断点以上（`up`）还是以下（`down`）的所有屏幕尺寸上隐藏。
...
| `.ion-no-padding`         | `padding: 0`            | 不为所有边应用内边距。          |

### 外边距

margin 区域通过一个空白区域扩展边框区域，用于将元素与其相邻元素分开。
...
| `.ion-no-margin`         | `margin: 0`            | 不为所有边应用外边距。          |
...
| 类                              | 样式规则                          | 描述                                                       |
| ------------------------------- | --------------------------------- | ---------------------------------------------------------- |
| `.ion-justify-content-start`    | `justify-content: flex-start`     | 项目在主轴方向上朝向起始端排列。                           |
| `.ion-justify-content-end`      | `justify-content: flex-end`       | 项目在主轴方向上朝向末端排列。                             |
| `.ion-justify-content-center`   | `justify-content: center`         | 项目在主轴方向上居中排列。                                 |
| `.ion-justify-content-around`   | `justify-content: space-around`   | 项目在主轴方向上均匀分布，周围空间相等。                   |
| `.ion-justify-content-between`  | `justify-content: space-between`  | 项目在主轴方向上均匀分布。                                 |
| `.ion-justify-content-evenly`   | `justify-content: space-evenly`   | 项目分布使得任意两个项目之间的间距相等。                   |
| `.ion-align-items-start`        | `align-items: flex-start`         | 项目在交叉轴方向上朝向起始端排列。                         |
| `.ion-align-items-end`          | `align-items: flex-end`           | 项目在交叉轴方向上朝向末端排列。                           |
| `.ion-align-items-center`       | `align-items: center`             | 项目在交叉轴方向上居中排列。                               |
...
| 类                         | 样式规则                  | 描述                                                   |
| -------------------------- | ------------------------- | ------------------------------------------------------ |
| `.ion-align-self-start`    | `align-self: flex-start`  | 项目在交叉轴方向上朝向起始端排列。                     |
| `.ion-align-self-end`      | `align-self: flex-end`    | 项目在交叉轴方向上朝向末端排列。                       |
| `.ion-align-self-center`   | `align-self: center`      | 项目在交叉轴方向上居中排列。                           |
| `.ion-align-self-baseline` | `align-self: baseline`    | 项目对齐方式使得其基线与其他项目的基线对齐。           |
| `.ion-align-self-stretch`  | `align-self: stretch`     | 项目拉伸以填充容器。                                   |
| `.ion-align-self-auto`     | `align-self: auto`        | 项目根据父元素的 `align-items` 值进行定位。            |

## 边框显示

`.ion-no-border` 工具类可用于从 Ionic 组件中移除边框。此类可应用于 `ion-header` 和 `ion-footer` 组件。
...
Ionic 在媒体查询中使用断点来根据屏幕大小对应用进行不同的样式设置。以下断点名称用于上述工具类中，当满足宽度条件时将应用该类。
