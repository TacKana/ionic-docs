...
<head>
  <title>手势 | 用于自定义手势和交互的 Ionic 应用实用程序</title>
  <meta
    name="description"
    content="Ionic Gestures 是一个实用程序，允许开发者以平台无关的方式为其应用构建自定义手势和交互。阅读了解更多。"
  />
</head>

## 概述

Ionic Gestures 是一个实用程序，允许开发者以平台无关的方式为其应用构建自定义手势和交互。开发者不需要使用特定的框架（如 React 或 Angular），甚至不需要构建 Ionic 应用！只要开发者可以访问 v5.0 或更高版本的 Ionic Framework，他们就可以使用所有 Ionic Animations。

构建复杂的手势可能很耗时。其他提供自定义手势的库通常过于笨重，最终会捕获鼠标或触摸事件而不让它们传播。这可能导致其他元素不再可滚动或可点击。
...
在此示例中，我们的应用监听 `.rectangle` 元素上的手势。当检测到手势移动时，会调用 `onMove` 函数，我们的应用记录当前的手势信息。
...
在上面的示例中，我们希望能够检测元素上的双击。通过将 `threshold` 设置为 `0`，我们可以确保手势对象能够检测点击。此外，我们定义了一个点击阈值，以便只有在快速连续发生的 2 次点击才算作双击。

<Codepen user="ionic" slug="oNvVEwE" />

## 手势动画

查看我们关于实现手势动画的指南：[使用 Ionic Animations 的手势动画](animations.md#gesture-animations)
...
| direction        | `'x' \| 'y' \| undefined`                  | `'x'`       | 将手势检测限制在沿某个轴的运动。                                                                                                 |
| gestureName      | `string`                                   | `undefined` | 要创建的手势名称。                                                                                                               |
| gesturePriority  | `number \| undefined`                      | `0`         | 优先级较高的手势将覆盖优先级较低的手势。有助于确保多个手势不会相互冲突。                                                         |
| passive          | `boolean \| undefined`                     | `true`      | 如果为 true，表示手势永远不会调用 `preventDefault()`。可用于提高滚动性能。参见[被动监听器](https://developer.mozilla.org/en-US/docs/Web/API/EventTarget/addEventListener#Improving_scrolling_performance_with_passive_listeners)了解更多信息。 |
| maxAngle         | `number \| undefined`                      | `40`        | 检测手势时允许的最大角度。                                                                                                      |
| threshold        | `number \| undefined`                      | `10`        | 定义指针在开始手势前必须移动多少。                                                                                               |
| blurOnStart      | `boolean \| undefined`                     | `undefined` | 如果为 true，手势将在触发 `onStart` 回调之前模糊任何活动可选元素，如 input 或 textarea。                                        |
...
| onEnd            | `GestureCallback \| undefined`             | `undefined` | 在手势结束时触发的回调。这通常在指针被释放时发生。                                                                               |
| notCaptured      | `GestureCallback \| undefined`             | `undefined` | 在手势未被捕获时触发的回调。这通常发生在有更高优先级的冲突手势时。                                                               |
...
| deltaX          | `number`           | 手势从开始以来在 x 轴上移动的距离。                                                 |
| deltaY          | `number`           | 手势从开始以来在 y 轴上移动的距离。                                                 |
| timeStamp       | `number`           | 手势的当前时间戳。                                                                  |
| event           | `UIEvent`          | 浏览器分发的原生事件。更多信息参见 [UIEvent](https://developer.mozilla.org/en-US/docs/Web/API/UIEvent)。 |
