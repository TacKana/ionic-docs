...
根据用户所在的平台，isPlatform(platformName) 将返回 true 或 false。请注意，同一个应用可以对多个平台名称返回 true。例如，在 iPad 上运行的应用将对以下平台名称返回 true：mobile、ios、ipad 和 tablet。此外，如果应用从 Cordova 运行，则 cordova 将返回 true。

## getPlatforms

`getPlatforms` 方法可用于确定你的应用当前在哪些平台上运行。
...
根据你所在的设备，`getPlatforms` 可以返回多个值。每个可能的值都是一个平台层次结构。例如，在 iPhone 上，它将返回 mobile、ios 和 iphone。
...
createApp(App).use(IonicVue, {
  platform: {
     ...
     * 这并不总是期望的行为，因此此函数改用 User Agent 进行检测。
     **/
    desktop: (win) => {
