...
<head>
  ...
  <meta
    name="description"
    content="Ionic Vue 快速入门涵盖 Vue 和 Ionic 的基础知识，包括任何 Ionic 特有的功能。了解如何使用 Ionic CLI 构建 Vue 应用。"
  />
</head>
...
欢迎！本指南将带你了解 Ionic Vue 开发的基础知识。你将学习如何搭建开发环境、生成简单项目、探索项目结构，以及了解 Ionic 组件的工作原理。这是在构建你的第一个真正应用之前熟悉 Ionic Vue 的完美方式。

如果你正在寻找 Ionic Vue 是什么以及它如何融入 Vue 生态系统的高级概述，请参阅 [Ionic Vue 概述](overview)。
...
这设置了应用的根结构，使用了 Ionic 的 `ion-app` 和 `ion-router-outlet` 组件。路由出口是你的页面将要显示的地方。
...
这将创建一个带有标题和可滚动内容区域的页面。第二个标题展示了一个[可折叠大标题](/api/title.md#collapsible-large-titles)，当位于内容顶部时显示，滚动时会折叠为第一个标题中的较小标题。
...
你可以使用更多 Ionic UI 组件来增强首页。例如，在 `ion-content` 的末尾添加一个[按钮](/api/button.md)：
...
这将创建一个在[工具栏](/api/toolbar.md)中带有[返回按钮](/api/back-button.md)的页面。返回按钮将自动处理导航到上一页，如果没有历史记录则导航到 `/`。

:::warning
创建自己的页面时，始终使用 `ion-page` 作为根组件。这对于页面之间的正确过渡、Ionic 组件所依赖的基础 CSS 样式以及应用中一致的布局行为至关重要。
:::

## 导航到新页面

要导航到新页面，首先需要为其创建路由。在 `router/index.ts` 中 `HomePage` 导入之后添加导入：
...
:::info
路由导航也可以使用 Vue Router 以编程方式执行，且路由可以懒加载以获得更好的性能。更多信息请参阅 [Vue 导航文档](/vue/navigation.md)。
:::
...
更新 `NewPage.vue` 中的导入以导入 `IonIcon` 以及 `heart` 和 `logoIonic` 图标：
...
请注意，我们传递的是导入的 SVG 引用，**不是**图标名称字符串。
...
更新 `NewPage.vue`，在现有图标之后添加 `ion-content` 的 ref、一个按钮和一些项目：
...
1. 为组件创建一个 `ref`
2. 通过 `$el` 访问底层 Web Component
3. 在 Web Component 上调用方法

这种模式是必要的，因为 Ionic 组件是作为 Web Component 构建的。`$el` 属性让你能够访问定义方法的实际 Web Component 实例。

每个组件的可用方法可以在其 API 文档的 [Methods](/api/content.md#methods) 部分找到。
...
Ionic Vue 项目默认使用 TypeScript 创建，但如果你愿意，可以轻松转换为 JavaScript。生成空白 Ionic Vue 应用后，请按照以下步骤操作：
...
2. 将所有 `.ts` 文件的扩展名改为 `.js`。在空白 Ionic Vue 应用中，这将包括 `src/router/index.ts`、`src/main.ts` 以及 `tests` 目录中的文件。

3. 在 `index.html` 中，将导入的 `<script>` 文件从 `/src/main.ts` 改为 `/src/main.js`。
...
6. 移除 Vue 组件中 script 标签上的 `lang="ts"`。在空白 Ionic Vue 应用中，这应该只有 `src/App.vue` 和 `src/views/HomePage.vue`。

7. 删除 `tsconfig.json` 和 `vite-env.d.ts`。

8. 在 package.json 中，将构建脚本从 `"build": "vue-tsc && vite build"` 改为 `"build": "vite build"`。

9. 安装 terser：`npm i -D terser`。

## 进一步探索

本指南涵盖了创建 Ionic Vue 应用、添加导航以及引入 Capacitor 进行原生构建的基础知识。要深入了解，请查看：
...
<DocsCard header="主题" href="/theming/basics" icon="/icons/guide-theming-icon.png">
  <p>了解如何使用 Ionic 强大的主题系统自定义应用的外观和感觉。</p>
</DocsCard>
