...
<head>
  <title>Router Link | ion-router-link 导航组件详解</title>
  <meta
    name="description"
    content="使用 ion-router-link 组件导航到指定链接。该路由链接可接受用于定位的 href 属性和用于转场动画的方向参数。"
  />
</head>
...
:::note
注意：此组件应仅用于原生 JavaScript 和 Stencil 项目。对于 Angular 项目，请使用 Angular 路由器的 `<a>` 标签和 `routerLink` 指令。
:::
